...
위의 users 테이블이 디스크에 놓이는 모습:

```text
행 지향 (MySQL):
[1, 김철수, 2026-01-15, KR] [2, Jane, 2026-02-20, US] [3, 田中, 2026-03-05, JP]

열 지향 (ClickHouse):
user_id 파일:      [1, 2, 3]
name 파일:         [김철수, Jane, 田中]
signup_date 파일:  [2026-01-15, 2026-02-20, 2026-03-05]
country 파일:      [KR, US, JP]
```

분석 쿼리 `SELECT count() FROM users WHERE country = 'KR'`을 실행하면:

- **행 지향**: 모든 행의 모든 컬럼을 통째로 읽어야 한다 (country만 필요한데도)
- **열 지향**: `country` 파일 **하나만** 읽으면 된다 → 읽는 데이터가 1/100 이하로 줄어든다
