...
이유: 첫 번째 키가 `user_id`면 'buy'가 100만 사용자 구간마다 흩어져 있어
건너뛸 granule이 없다. 첫 번째 키가 `event_type`이면 'buy'가 한 구간에 모여 있다.
앞 키로 정렬된 큰 구간 안에서만 뒷 키가 정렬되므로, **뒤 키 단독 필터는 효과가 약하다.**
