...
이 장 전체를 결정 트리 하나로 요약하면 다음과 같다. 시험장에서 컬럼을 볼 때마다
이 트리를 머릿속에서 타면 된다:

![컬럼 타입 선택 결정 트리 — 정체(정수·문자열·시간·소수)를 정하고, 각 갈래의 질문 하나로 최종 타입에 도달한다](../docs/assets/diagrams/type-decision.svg)
...
```quiz
Q: 사용자 ID(최대 수억 명, 음수 없음)에 가장 효율적인 타입은?
1) Int64
2) UInt32 *
3) String
E: 음수가 없으면 UInt, 최댓값(수억)을 여유 있게 덮는 최소 크기는 UInt32(약 42.9억)다. "표현 범위를 만족하는 가장 작은 타입"이 채점 기준이다 (4.1절).
```

```quiz
Q: `SELECT toInt8(200)`의 결과는?
1) 200
2) 에러 발생
3) -56 (조용한 오버플로) *
E: to계열 변환은 범위를 넘으면 에러 없이 값이 깨진다 (26.8 실측). 안전하게 하려면 `accurateCast(200, 'Int8')` — 이쪽은 에러를 던진다 (4.1절).
```

```quiz
Q: 로그 레벨('INFO','WARN','ERROR' 3종) 컬럼에 권장되는 타입은?
1) String
2) LowCardinality(String) *
3) FixedString(5)
E: 고유값이 적은 문자열은 LowCardinality로 감싸면 사전 인코딩되어 저장·비교가 극적으로 빨라진다. 실측 압축률 208배 (4.4절).
```

```quiz
Q: LowCardinality를 쓰면 안 되는 컬럼은?
1) 국가 코드 (~250종)
2) 세션 토큰 (수백만 종) *
3) HTTP 메서드 (10종 미만)
E: 고유값이 10만을 넘으면 일반 String보다 오히려 느려질 수 있다. user_id·URL·이메일·토큰은 금지 대상 (4.4절).
```

```quiz
Q: 주문 금액(통화)에 권장되는 타입은?
1) Float64
2) Decimal(18, 2) *
3) UInt64에 센트 단위로... 도 가능하지만 표준은 아님
E: 돈은 이진 부동소수점 오차가 허용되지 않으므로 Decimal이다. 단 sum(Decimal(18,2))의 결과 타입은 Decimal(38,2)로 커진다는 것까지 기억하자 (4.2절).
```
