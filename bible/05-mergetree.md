...
```quiz
Q: MergeTree 테이블에 INSERT를 한 번 실행하면 무슨 일이 일어나는가?
1) 기존 데이터 파일에 행이 끼워 넣어진다
2) 새 part(불변 조각)가 하나 생성된다 *
3) 테이블 전체가 다시 정렬된다
E: INSERT 한 번 = part 하나. 기존 part는 절대 수정되지 않고, 백그라운드 merge가 나중에 합친다. 그래서 배치 삽입이 원칙이다 (5.2절).
```
...
2) 8,192행 *
3) 65,536행
E: index_granularity 기본값 8192. sparse index는 granule마다 첫 행의 키 하나만 기록해 10억 행도 인덱스가 메모리에 들어간다 (5.3절).
```

```quiz
Q: ClickHouse의 primary key에 대한 올바른 설명은?
1) 같은 키의 행은 자동으로 거부된다
2) 유일성을 보장하지 않는다 — 같은 키의 행이 공존할 수 있다 *
3) B-tree로 모든 행의 위치를 기록한다
E: OLTP와 결정적으로 다른 부분. 중복 제거가 필요하면 14장의 ReplacingMergeTree를 쓴다 (5.4절).
```

```quiz
Q: `EXPLAIN indexes=1`의 출력이 `Granules: 12/12`라면?
1) 인덱스가 완벽하게 작동 중
2) 풀스캔 — ORDER BY 키로 필터하지 않았다는 신호 *
3) 12개의 인덱스가 사용됨
E: 읽은수/전체수가 같으면 하나도 건너뛰지 못한 것이다. 쿼리 또는 키 설계를 재검토하라 (5.3절, 16장).
```

```quiz
Q: PARTITION BY의 올바른 용도는?
1) 쿼리 속도를 높이는 인덱스
2) 오래된 데이터 삭제·이동 같은 수명 관리 *
3) 중복 제거
E: 파티션은 관리 도구다. 공식 실측에서 파티션 키와 무관한 쿼리는 파티션 없는 테이블(0.012초)보다 오히려 느렸다(0.090초) (5.5절).
```
