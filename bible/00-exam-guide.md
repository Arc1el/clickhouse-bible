...
![시험 5개 영역과 이 책의 장 매핑 — 모든 과제가 이 다섯 상자 중 하나에서 나온다](../docs/assets/diagrams/exam-map.svg)
...
```quiz
Q: ClickHouse Certified Developer 시험의 형식은?
1) 객관식 60문항
2) 실제 클러스터에서 과제 10~12개를 수행하는 실기 *
3) 서술형 논술
E: 객관식·OX 문제는 하나도 없다. HackerRank에서 실제 ClickHouse 환경에 접속해 테이블을 만들고 쿼리를 짜는 실기 시험이다 (0.2절).
```

```quiz
Q: 시험 중에 열어볼 수 있는 것은?
1) 아무것도 볼 수 없다
2) clickhouse.com 도메인 전체 (문서·블로그) *
3) 모든 웹사이트와 AI 도구
E: clickhouse.com 도메인(문서+블로그)은 허용되지만, AI 도구는 문서 사이트의 "Ask AI" 기능까지 포함해 전면 금지다 (0.2절).
```

```quiz
Q: 합격 기준과 결과 통지는?
1) 60% 이상, 즉시 통지
2) 70% 이상, 최대 5영업일 (자동 채점 + 수동 검토) *
3) 80% 이상, 24시간 이내
E: 70%가 합격선이고, 수동 검토가 병행되어 결과는 최대 5영업일 걸린다 (0.2절).
```

```quiz
Q: 불합격 후 재응시하려면?
1) 7일 대기 후 $200을 다시 결제하면 횟수 무제한 *
2) 30일 대기, 최대 3회
3) 재응시 불가
E: 재응시 횟수는 무제한이지만 직전 응시로부터 7일을 기다려야 하고 매회 $200이다. 응시권은 구매 후 365일 내 아무 때나 쓸 수 있다 (0.2절).
```
